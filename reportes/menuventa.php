...
<?php
$sql = "SELECT * FROM listaventa inner join cliente on idCliente = id_clientes  inner join producto on idProducto = id_producto where status = 1";
...
";
    ...
    $id=$row['idListaVenta'];
        ...
        if ($row["iva"]==1)
        {
           echo "<td>$".$row["totalCobrar"]*0.16."</td> "; 
           echo "<td>$".$row["totalCobrar"]*1.16."</td> "; 
        }
        else
        {
            echo "<td>$". 0 ."</td> ";
            echo "<td>$".$row["totalCobrar"]."</td> ";
        } ?>
        <td><a <?php echo "href='../bascula/boletaventa.php?id=$id' "?> ><button type="button" class="btn btn-large btn-primary" data-toggle="modal" data-target="#ModalEditar"><i class='fa fa-print'></i> Reimprimir boleta</button></a>
        </td>
        <?php if ($row['totalCobrar'] != 0 && $row['iva'] == 0){ ?>
        <td><a <?php echo "href='../caja/ticketventa.php?id=$id' "?> ><button type="button" class="btn btn-large btn-primary" data-toggle="modal" data-target="#ModalEditar"><i class='fa fa-print'></i> Reimprimir recibo</button></a>
        </td>
     <?php }
      else
      {
